...
LPDIRECT3DDEVICE9	CDirectTest::lpD3DDevice=NULL;
CRITICAL_SECTION	CDirectTest::csLoadTexture;
                   ...
                   LPSTR lpCmdLine, int nCmdShow)
{
	...
	DTest.SetHandle(&hWnd,&hInstance,nCmdShow);
	StartUp();
	...
	while(WM_QUIT != msg.message)
	{
		if(PeekMessage(&msg,NULL,0,0,PM_REMOVE))
		{
			...
			while(time.QuadPart-oldtime.QuadPart< gOneFrame*frec.QuadPart/1000){
				list<HANDLE>::iterator it = DTest.mHThread.begin();
				while( it != DTest.mHThread.end() )
				{
					if(WaitForSingleObject( *it, 0 ) == WAIT_OBJECT_0){
						CloseHandle( *it );
						it = DTest.mHThread.erase(it);
					}else{
						++it;
					}
				}
			...
			if(gDrawSkip){
				DTest.OnTimer();
				oldtime.QuadPart+=LONGLONG(gOneFrame*frec.QuadPart/1000);
				if(time.QuadPart<oldtime.QuadPart+gOneFrame*frec.QuadPart/1000){
					DTest.OnDraw();
				}
			}else{
				QueryPerformanceCounter(&oldtime);
...
bool MakeWindow(HINSTANCE *hInstance,HWND *hwnd,int nCmdShow)
{
    ...
    wcex.hIconSm        = LoadIcon(*hInstance, "IDI_ICON");
...
CDirectTest::CDirectTest(void)
{
	...
	mFPSCount=0;
	mOldTime=0;
	InitializeCriticalSection(&csLoadTexture);
//	InitializeCriticalSection(&csAccessTexture);
}
CDirectTest::~CDirectTest(void)
{
//	DeleteCriticalSection(&csAccessTexture);
	
	timeEndPeriod(1);
	Release(true);
	DeleteCriticalSection(&csLoadTexture);
}
void CDirectTest::SetHandle(HWND *hWnd,HINSTANCE* hInstance,int nCmdShow)
{
	mHWnd=hWnd;
...
bool CDirectTest::MakeD3D(HWND *hWnd,HINSTANCE* hInstance,int nCmdShow)
{
	...
	param.SwapEffect			= D3DSWAPEFFECT_COPY;	 // 表示する方
	...
	result=lpD3D->CreateDevice(
		...
		D3DDEVTYPE_HAL,                     
		*hWnd,                               // ウインドウハンドル
		D3DCREATE_SOFTWARE_VERTEXPROCESSING | D3DCREATE_MULTITHREADED, // ソフトウェアによる頂点処理
	...
	*mHWnd=0;
	...
	while(tex){
		if(tex->mRenderTarget){
			...
			tex->SetTexture(priCreateTexture(tex->mFileName,tex->mColor));
		}
		tex=tex->mNextList;
	}
...
//	tex->lpDirectTexture=CreateTexture(tex->mWidth,tex->mHeight);
	if(!tex->mBuffer){
		tex->SetTexture(priCreateTexture(tex->mWidth,tex->mHeight));
		return;
	}
...
bool CDirectTest::Release(bool deltex)
{
	...
	if(CDTFont::mSprite){
		CDTFont::mSprite->End();
		CDTFont::mSprite->Release();
		CDTFont::mSprite=NULL;
	}
...
#ifdef _SET_FPS_
	for(int fps=0;fps<_SET_FPS_;fps++){
#endif
	GetInputData();
	::OnTimer();
#ifdef _SET_FPS_
...
{
	if(!mInited)	return;
//	EnterCriticalSection(&csAccessTexture);

	DWORD time=timeGetTime();
	if(mOldTime>0){
		mFPS[mFPSCount%FPS_COUNT_NO]=time-mOldTime;
		mFPSCount++;
	}
	mOldTime=time;
	DrawStart();
...
//	LeaveCriticalSection(&csAccessTexture);
}
...
bool CDirectTest::DrawPolygon(CPolygon *poly,int no)
{
	EndTextMode();
	lpD3DDevice->SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE);
	CV_2D *cv = new CV_2D[no];
	...
	lpD3DDevice->SetTexture( 0, NULL );
	lpD3DDevice->SetFVF( D3DFVF_XYZRHW|D3DFVF_DIFFUSE|D3DFVF_TEX1 );
	lpD3DDevice->DrawPrimitiveUP( D3DPT_TRIANGLEFAN, no-2, cv, sizeof(CV_2D) );

	delete cv;
	return true;
}
bool CDirectTest::DrawLine(double fx,double fy,double tx,double ty,double width,D3DCOLOR color)
{
...
bool CDirectTest::CreateTexture(CDTTexture &texture,int x,int y)
{
	...
	lpD3DTexture=priCreateTexture(tx,ty);
...
LPDIRECT3DTEXTURE9 CDirectTest::priCreateTexture(int x,int y)
{
	LPDIRECT3DTEXTURE9 lpD3DTexture;
...
LPDIRECT3DTEXTURE9 CDirectTest::priCreateTexture(const char *filename,D3DCOLOR color,int *x,int *y)
{
	LPDIRECT3DTEXTURE9 lpD3DTexture;
...
unsigned int CDirectTest::MTLoadTexture( void *p )
{
	EnterCriticalSection(&csLoadTexture);
	CTexInfo* info=(CTexInfo*) p;
	...
	int filesize;
	D3DCOLOR* c=NULL;
	if(info->color==TRC_ZERO){
		c=&(info->color);
	}
	if(!GetFixedBMP(info->filename,&bmpdata,&x,&y,&filesize,c)){
		info->texture->SetLoaded(true);
		delete info;
		LeaveCriticalSection(&csLoadTexture);
	...
	HRESULT result=D3DXCreateTextureFromFileInMemoryEx(lpD3DDevice,bmpdata,filesize,0,0,1,0,D3DFMT_A8R8G8B8,D3DPOOL_MANAGED,D3DX_DEFAULT ,D3DX_DEFAULT ,info->color,NULL,NULL,&lpD3DTexture );
	if(D3D_OK==result)
	{
		//EnterCriticalSection(&csAccessTexture);
		info->texture->SetTexture(lpD3DTexture,info->filename,x,y,false,info->color);
		//LeaveCriticalSection(&csAccessTexture);
#ifdef LOG
	}else{
		FILE* pLog;
		pLog=fopen("log.txt","a");
		if(pLog){
			fputs("ERROR:LoadTexture\n",pLog);
			switch(result){
				case D3DERR_NOTAVAILABLE:
					fputs("      NOTAVAILABLE\n",pLog);		break;
				case D3DERR_OUTOFVIDEOMEMORY:
					fputs("      OUTOFVIDEOMEMORY\n",pLog);	break;
...
bool CDirectTest::LoadTexture(CDTTexture &texture,const char *filename,D3DCOLOR color)
{
	...
	if(texture.ReleaseForLoadTexture(filename)){
		if(!(texture.SeachTexture(filename))){
			CTexInfo *info = new CTexInfo;
			info->texture  = &texture;
			strcpy(info->filename,filename);
			info->color    = color;

			uintptr_t ret;
			//ret = _beginthread( CDirectTest::MTLoadTexture, 0, (void *) (info)  );
			unsigned int id;
			...
			ret=_beginthreadex( NULL,0,CDirectTest::MTLoadTexture,  (void *) (info)  ,0,&id);
			if(ret!= -1L) {
				//SetThreadPriority(HANDLE(ret),THREAD_PRIORITY_ABOVE_NORMAL);
				mHThread.push_back(HANDLE(ret));
			}
...
bool CDirectTest::DrawTextureRotate(CDTTexture &texture,int turn,int fx,int fy,int fsx,int fsy,int tx,int ty,int tsx,int tsy,D3DCOLOR color1,D3DCOLOR color2,D3DCOLOR color3,D3DCOLOR color4,double r)
{
	EndTextMode();
	...
	float arctan=atan2(float(tsy),float(tsx));
	float l=float(_hypot(tsx/2.0,tsy/2.0));
	float p=3.141592f;
	...
	cv[0].x = tx+cos(p+arctan+rad)*l;		cv[0].y = ty+sin(p+arctan+rad)*l;		cv[0].tu = fleft;	cv[0].tv = ftop;	cv[0].z = 0.5f; cv[0].rhw = 1.0f;	 cv[0].color = color1;	
	cv[1].x = tx+cos(-arctan+rad)*l;		cv[1].y = ty+sin(-arctan+rad)*l;		cv[1].tu = fright;	cv[1].tv = ftop;	cv[1].z = 0.5f; cv[1].rhw = 1.0f;	 cv[1].color = color2;			
	cv[2].x = tx+cos(p-arctan+rad)*l;		cv[2].y = ty+sin(p-arctan+rad)*l;		cv[2].tu = fleft;	cv[2].tv = fbottom;	cv[2].z = 0.5f; cv[2].rhw = 1.0f;	 cv[2].color = color3;			
	cv[3].x = tx+cos(arctan+rad)*l;			cv[3].y = ty+sin(arctan+rad)*l;			cv[3].tu = fright;	cv[3].tv = fbottom;	cv[3].z = 0.5f; cv[3].rhw = 1.0f;	 cv[3].color = color4;			
...
bool CDirectTest::DrawTexture(CDTTexture &texture,int turn,int fx,int fy,int fsx,int fsy,double tx,double ty,double tsx,double tsy,D3DCOLOR color1,D3DCOLOR color2,D3DCOLOR color3,D3DCOLOR color4)
{
	EndTextMode();
	...
	cv[0].x = tleft;	cv[0].y = ttop;		cv[0].tu = fleft;	cv[0].tv = ftop;	cv[0].z = 0.5f; cv[0].rhw = 1.0f;	 cv[0].color = color1;	
	cv[1].x = tright;	cv[1].y = ttop;		cv[1].tu = fright;	cv[1].tv = ftop;	cv[1].z = 0.5f; cv[1].rhw = 1.0f;	 cv[1].color = color2;			
	cv[2].x = tleft;	cv[2].y = tbottom;	cv[2].tu = fleft;	cv[2].tv = fbottom;	cv[2].z = 0.5f; cv[2].rhw = 1.0f;	 cv[2].color = color3;			
	cv[3].x = tright;	cv[3].y = tbottom;	cv[3].tu = fright;	cv[3].tv = fbottom;	cv[3].z = 0.5f; cv[3].rhw = 1.0f;	 cv[3].color = color4;			
...
*/	}
	return false;
}void CDirectTest::StartTextMode(int no)
{
	if((mTextMode>=0)!=(no>=0)){
		if(mTextMode>=0){
			EndTextMode();
		}else{
			if(CDTFont::mSprite)	CDTFont::mSprite -> Begin(0);
			mTextMode=no;
		}
	}
}
...
{
	if(mTextMode>=0){
		if(CDTFont::mSprite)	CDTFont::mSprite -> End();
		mTextMode=-1;
	}
}
bool CDirectTest::CreateFont(int fontnum,int size,const char* fontname,int weight,bool italic)
{
	...
	LPD3DXFONT font;
	D3DXCreateFont(lpD3DDevice,size,0,weight,0,italic,SHIFTJIS_CHARSET,OUT_TT_ONLY_PRECIS,DEFAULT_QUALITY,0,fontname,&font);
...
bool CDirectTest::DrawText(int fontnum,const char *text,int x,int y,D3DCOLOR color,int position,int sizex,int sizey)
{
	...
	if(sizex<0)	textspace.right=mTargetWidth;
	if(sizey<0)	textspace.bottom=mTargetHeight;
	font->DrawText(mFont[fontnum].mSprite,text,-1,&textspace,position,color);
...
void CDirectTest::SetIcon(LPCSTR icon)
{
	HICON hIcon;
	hIcon = LoadIconA(*mHInstance, icon);
	SendMessage(*mHWnd, WM_SETICON, ICON_BIG, (WPARAM)hIcon);
	SendMessage(*mHWnd, WM_SETICON, ICON_SMALL, (WPARAM)hIcon);
}
...
double CDirectTest::GetFPS(void)
{
	DWORD time=0;
	int no=mFPSCount;
	if(no>FPS_COUNT_NO)	no=FPS_COUNT_NO;
	for(int i=0;i<no;i++){
		time+=mFPS[i];
	}
	return 1000.0*no/time;
}
