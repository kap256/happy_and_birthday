...
void CMapObject::OnDraw(int cx,int cy)
{
	...
	DTest.DrawTexture(mImg,turn,tx*64,ty*64,64,64,drawx,drawy,MAP_CHIP_SIZE*2,MAP_CHIP_SIZE*2,mColor);
...
int CMapObject::OnTimer(int mapsizex,int mapsizey,CMapMode *map,CMapObject* object[OBJECT_NO])
{
	...
	if(mColorTimer>0){
		...
		if(mColorTimer<=0){
			...
			unsigned char* goal=(unsigned char*)&mGoalColor;
			for(int i=0;i<4;i++){
				c[i]+=(goal[i]-c[i])/mColorTimer;
			}
		}
		mColorTimer--;
	}
	...
	if(!mGround){
		...
		if(mZ>=0){
			if(	(map->GetChipType(gravityx,gravityy,int(mZ)/16) & MCHIP_V_GROUND) &&mSpeedZ<0){
				mGround=true;
			}
			if(map->GetChipType(int(mX+16)/32,int(mY+16)/32,int(mZ)/16) & MCHIP_V_GROUND){
				mGround=true;
			}
		}
		if (!mGround && mHit){
		...
		if(((x!=0)||(y!=0))&&(mGround)){
			...
			for(int i=0;i<3+jump;i++){
				if(map->GetChipType(GetX_32(),GetY_32(),nz+i) & MCHIP_FENCE){
					int chip=map->GetChip(GetX_32(),GetY_32(),nz+i)%4;
					if(chip<2){
						if(mDir>=2){		space=0;	break;}
					}else if(chip%4==mDir){
						space=0;	break;
					}
				}
				if(map->GetChipType(nx,ny,nz+i) & MCHIP_FENCE){
					int chip=map->GetChip(nx,ny,nz+i)%4;
					if(chip<2){
						if(mDir<2){	space=0;	break;}
					}else if((chip+2)%4==mDir){
						space=0;	break;
					}
				}
				if(map->GetChipType(nx,ny,nz+i) & MCHIP_H_GROUND){
					space=0;
					height=i+1;
				}else{
					space++;
			...
			if(space<3){
				...
				mFloatTimer=0;
				if(bgrav==GRAV_JUMP){
					mSpeedZ=(3.5+mSpeed/16.0)*pow(double(height),0.7);
				}else{
					mSpeedZ=3.0;
				}
				nz+=height;
			...
			if (mHit){
				for(int ObjNo=0;ObjNo<OBJECT_NO;ObjNo++){
					if (object[ObjNo] && object[ObjNo]->CheckHit(this,true,nz*16)>0){
						mGoalX=GetX_32();
						mGoalY=GetY_32();
						mSpeedZ=0.0;
			...
			for(h=nz;h>=0;h--){
				...
				bool moveok=((MapChipParam[chip/20] & nomove)!=0);
				if(mGravity & GRAV_WATER_ONLY){
				...
				if(chip==1 || moveok){
					...
					return -4;
				}else if(MapChipParam[chip/20] & MCHIP_V_GROUND){
...
void	CMapObject::SetColor(D3DCOLOR color,int time)
{
	mGoalColor=color;
	if(time<=0){
		mColor=color;
...
int CMapObject::CheckHit(CMapObject* obj,bool walk,int z)
{
	int ret=-1;
	int x,y;
	if(z<0){
...
int CPlayerObject::CheckHit(CMapObject* obj,bool walk,int z)
{
	bool enemy=(obj->GetObjType()==MapObjEnemy);

	if(enemy && mWalkTimer>0)
		walk=false;
	return CMapObject::CheckHit(obj,walk,z);
}
...
{
	InitObject(id,x,y,z,dir,32,0,100,true,GRAV_NO_JUMP,1 | OBJOPT_NO_SHADOW);
...
bool CBoxObject::CheckTalk(CMapObject* obj)
{
	if(CheckHit(obj,true)!=0){
		...
		MapMode.SendMessage(EH_TALK,mID,-1);
		return true;
	}
	return false;
}
...
int CHighObject::CheckHit(CMapObject* obj,bool walk,int z)
{
	int ret=-1;
	int x=obj->GetX_32();
	int y=obj->GetY_32();
...
int CWoodObject::CheckHit(CMapObject* obj,bool walk,int z)
{
	bool player=(obj->GetID()==0);
	int ret=CHighObject::CheckHit(obj,walk,z);
...
int CBigObject::CheckHit(CMapObject* obj,bool walk,int z)
{
	int ret=-1;
	int x=obj->GetX_32();
	int y=obj->GetY_32();
...
int CPutObject::CheckHit(CMapObject* obj,bool walk,int z)
{
	bool player=(obj->GetID()==0);

	int hit=CLiftObject::CheckHit(obj,walk,z,1);

	if(hit>0 && walk && mWalkTimer<=0 && player && mHeavy<=0){
		bool move=true;
		if(mParam & OBJOPT_HEAVY_1){
			if(!Flag.SearchFSkill(296))	move=false;
		}
		if(mParam & OBJOPT_HEAVY_2){
			if(!Flag.SearchFSkill(297))	move=false;
		}
...
int		CLiftObject::CheckHit(CMapObject* obj,bool walk,int z)
{
	return CheckHit(obj,walk,z,0);
}

int		CLiftObject::CheckHit(CMapObject* obj,bool walk,int z,int bottomhit)
{
	int ret=-1;
	int x=obj->GetX();
	int y=obj->GetY();
	...
	if(z<=mZ && bottomhit){
		ret=CMapObject::CheckHit(obj,walk,z);
		if(ret<=0)	return ret;
	}else{
		if(x<=mX-16)		return 0;
		if(x>mX+16)			return 0;
...
int		CNormalTrapObject::CheckHit(CMapObject* obj,bool walk,int z)
{
	bool player=(obj->GetID()==0);

	int hit=CMapObject::CheckHit(obj,walk,z);
...
int CEnemyObject::CheckHit(CMapObject* obj,bool walk,int z)
{
	bool player=(obj->GetID()==0);
...
int	 CEnemyObject::OnTimer(int mapsizex,int mapsizey,CMapMode *map,CMapObject* object[OBJECT_NO])
{
	...
	switch(mMoveMode){
		...
		case 12:Search_12(1,map,object);break;
		case 13:Search_12(3,map,object);break;
		case 14:Search_14(object);		break;
		case 15:Search_15(object);		break;
...
void	CEnemyObject::Search_0(CMapObject* object[OBJECT_NO])
{
	...
	if(Flag.mPlayTime%8==0){
		if(Flag.SearchFSkill(284)){
			if(pInput->Rand(8)!=0){
				if((mParamA!=0)&&(pInput->Rand(4)==0)){
					SetGoal(GetX_32(),GetY_32(),GetZ_32());
					mParamA=0;
				}
				return;
...
void	CEnemyObject::Search_12(int dir,CMapMode *map,CMapObject* object[OBJECT_NO])
{
// mParamA・・・主人公を発見したかどうか
// mParamB・・・タイマ
	if(mParamA){//発見しているとき
		mParamB++;
		if(mParamB>8){
		...
		int dx,dy;
		GetComp(GetDir(),&dx,&dy);
		int mx,my;
		map->GetMapSize(&mx,&my);
		...
		while(x>0 && x<mx && y>0 && y<my ){
			int ctype=map->GetChipType(x,y,z+1);
			if(ctype & MCHIP_H_GROUND)	return;
			for(int i=1;i<OBJECT_NO;i++){
				if(!object[i])	continue;
				int ox=object[i]->GetX_32();
				int oy=object[i]->GetY_32();
				int oz=object[i]->GetZ_32();
				if(ox==x && oy==y && oz-1<=z && z<=oz+1)	return;
			}
			int ox=object[0]->GetX_32();
			int oy=object[0]->GetY_32();
			int oz=object[0]->GetZ_32();
			if(ox==x && oy==y && oz-2<=z && z<=oz+2){
				mParamB=0;
				mParamA=1;
				SetZSpeed(4.0);
			}
...
void	CEnemyObject::Search_14(CMapObject* object[OBJECT_NO])
{
	int x=GetX_32();
	int y=GetY_32();
	int tx=object[0]->GetX_32();
	...
	if(CheckGoal()){
		if(lx*lx<=ly*ly){
			SetGoal(x,ty+ly,GetZ_32());
		}else{
			SetGoal(tx+lx,y,GetZ_32());
		}
	}else if(x==tx){
		SetGoal(x,ty+ly,GetZ_32());
	}else if(y==ty){
...
void	CEnemyObject::Search_15(CMapObject* object[OBJECT_NO])
{
	int l=mSearch*mSearch;
	if(Flag.mPlayTime%8==0){
		if(Flag.SearchFSkill(284)){
			if(pInput->Rand(8)!=0){
				SetGoal(mStartX,mStartY,GetZ_32());
			}
		}
		int tx=object[0]->GetX_32();
		int ty=object[0]->GetY_32();
...
void CMapObject::DrawDebugData(int x,int y)
{
	x+=16;
		...
		if(MapMode.mDebugShowImgNo){
			y+=16;
			DTest.DrawBox(x,y,x+40,y+10,0x80ffffff);
			DTest.Printf(1,x,y,"Img.%03d",mImgNo);
		}
	#endif
	#ifdef _SHOW_OUT_OBJ_
		int cx=GetX_32();
		int cy=GetY_32();
		int cz=GetZ_32();
		bool draw=false;
		if(MapMode.GetChipType(cx,cy,cz) & MCHIP_GROUND)	draw=true;
		if(cz<0)											draw=true;
		if(draw){
			int dx=320+mID/20*40;
			int dy=40+mID%20*20;
			DTest.DrawBox(dx,dy,dx+40,dy+20,0xffff8080);
			DTest.Printf(0,dx,dy,"%02d",mID);
		}

	#endif
}
void CMapObject::DrawDebugMapBMP(int x,int y)
{
}
...
void CBoxObject::DrawDebugMapBMP(int x,int y)
{
	int drawx;
	int drawy;
	...
	GetDrawInfo(drawx,drawy,tx,ty,turn);
	drawx+=(24*_OUTPUT_MAP_BMP_SIZE_/32)-x;
	drawy+=(16*_OUTPUT_MAP_BMP_SIZE_/32)-y;

	DTest.DrawBox(drawx,drawy,drawx+32,drawy+24,0xffcc0000);
	DTest.DrawBox(drawx+2,drawy+2,drawx+30,drawy+22,0xffffdddd);

	char text[32];
	for(int m=0;m<MAX_ALL_MONTH;m++){
		CMonthData &md=MonthData[m];
		for(int i=0;i<md.mTresureNo;i++){
			if(md.mTresure[i]==mBoxNo){
				sprintf(text,"%d",i+1);
				goto break_month ;
			}
		}
...
break_month:
	...
	DTest.DrawText(6,text,drawx,drawy,0xffcc0000,DT_CENTER,32,24);
}


void CEnemyObject::DrawDebugData(int x,int y)
{
	x+=16;
		...
		switch(lv/4){
			...
			case 3:	color=0xff8000ff;	break;
			default:color=0xffff8000;	break;
		}
		char text[32];
		sprintf(text,"ELv.%d",lv);
		DTest.DrawText(1,text,x,y,color);
	#endif
	CMapObject::DrawDebugData(x-16,y);
